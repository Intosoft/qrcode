...
(() => {
  ...
  // src/eyeball.ts
  var circleEyeballPath = ({ x, y, radius }) => {
    return `M${x + radius},${y}A${radius},${radius},0,1,1,${x - radius},${y},${radius},${radius},0,0,1,${x + radius},${y}Z`;
  };
  var circleEyeball = ({
    matrixLength,
    size
...
`;
  ...
  var circleEyeFramePath = ({
    ...
    return `M${x + radius},${y + length}h0A${radius},${radius},0,0,1,${x},${y + length / 2}h0A${radius},${radius},0,0,1,${x + radius},${y}h0A${radius},${radius},0,0,1,${x + length},${y + length / 2}h0A${radius},${radius},0,0,1,${x + radius},${y + length}Z`;
  };
  var circleEyeFrame = ({
    matrixLength,
    size
  ...
  var squareEyeFramePath = ({
    cellSize,
    length,
    x,
    y
  }) => {
    return `M${x + cellSize / 2},${y + cellSize / 2}H${x + length - cellSize / 2}V${y + length - cellSize / 2}H${x + cellSize / 2}Z`;
  };
  var squareEyeFrame = ({
    matrixLength,
    size
  ...
  var roundedEyeFramePath = ({
    cellSize,
    length,
    x,
    y
  }) => {
    const dynamic1 = cellSize / 2;
    const dynamic2 = cellSize * 1.1905;
    const dynamic3 = dynamic2 + dynamic1;
    const dynamic4 = length - dynamic3;
    const dynamic5 = dynamic4 - dynamic3;
    return `M${x + dynamic4},${y + length - dynamic1}H${x + dynamic3}A${dynamic2},${dynamic2},0,0,1,${x + dynamic1},${y + length - dynamic3}V${y + dynamic3}A${dynamic2},${dynamic2},0,0,1,${x + dynamic3},${y + dynamic1}h${dynamic5}A${dynamic2},${dynamic2},0,0,1,${x + length - dynamic1},${y + dynamic3}v${dynamic5}A${dynamic2},${dynamic2},0,0,1,${x + length - dynamic3},${y + length - dynamic1}Z`;
  };
  var roundedEyeFrame = ({
    matrixLength,
    size
  ...
  // src/path/index.ts
  var generatePath = ({ size, matrix, config }) => {
  ...
  // src/index.ts
  var quietZone = 0;
  var enableLinearGradient = true;
  var gradientDirection = ["0%", "0%", "100%", "100%"];
  var linearGradient = ["black", "black"];
  ...
  var generateSVGString = (paramConfig = defaultConfig) => {
    const config = {
      ...defaultConfig,
      ...paramConfig
    };
    ...
    const svg = `<svg viewBox="${[
      ...
      config.length + quietZone * 2
    ].join(" ")}" width="${config.length}" height="${config.length}" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="grad" x1="${gradientDirection[0]}" y1="${gradientDirection[1]}" x2="${gradientDirection[2]}" y2="${gradientDirection[3]}">
        <stop offset="0" stop-color="${linearGradient[0]}" stop-opacity="1" />
        <stop offset="1" stop-color="${linearGradient[1]}" stop-opacity="1" />
      </linearGradient>
    </defs>
    <g>
      <rect x="${-quietZone}" y="${-quietZone}" width="${config.length + quietZone * 2}" height="${config.length + quietZone * 2}" fill="${config.backgroundColor}" />
    </g>
    <g fill="${enableLinearGradient ? "url(#grad)" : config.color}"  stroke="${enableLinearGradient ? "url(#grad)" : config.color}">
  <path d="${path}" 
     stroke-linecap="butt" 
     stroke-width="${0}" /> 
     ${config.eyeFrameShape !== "circle-item" ? `<path
      fill="none"
     ...
     ${config.eyeballShape !== "circle-item" ? `<path
  ...
  if (window) {
    window.IntosoftQRCode = { generateSVGString };
  }
})();
